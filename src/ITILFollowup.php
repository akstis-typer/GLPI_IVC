...
class ITILFollowup extends CommonDBChild
{
    ...
    public function prepareInputForAdd($input)
    {
        ...
        if (isset($input["add_reopen"]) && !isset($input['_mailgate'])) { // Проверку && !isset($input['_mailgate']) добавили после того, как у одного письма не получилось добавиться в комментарий к тикету IVC 04.03.26 номер тикета №2838
            if ($input["content"] == '') {
                if (isset($input["_add"])) {
