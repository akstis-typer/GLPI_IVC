...
class RuleTicketCollection extends RuleCommonITILObjectCollection
{
    ...
    public function prepareInputDataForProcess($input, $params)
    {
        ...
        //If there is more than one reciever changed by Karpov N.N. 22.12.25 IVC 1339
        if(isset($input['_head']['tos'])) {
            for($i = 1; $i < count($input['_head']['tos']); $i++) {
                $to = $input['_head']['tos'][$i];
                $input['_to'] .= ", $to";
            }
        }
